...
var PREINSTALL_SCRIPT = "npm-workspace:preinstall";
  ...
  if(pkg) {
    ret = when(ret, function() {
      return self.installModule(cwd, wsDesc, pkg, installed)
        .then(() => {
          self.postInstallModule(pkg, cwd);
        });
    });
  }
 ...
 * Runs the npm-workspace post install script if present in the module's package
 */
self.preInstallModule = function(packageDescriptor, cwd) {
  if (!packageDescriptor.scripts || !packageDescriptor.scripts[PREINSTALL_SCRIPT]) {
    return;
  }
  self.log.info("npm run " + PREINSTALL_SCRIPT + " for " + cwd);
  self.npm(["run", PREINSTALL_SCRIPT], cwd);
}
