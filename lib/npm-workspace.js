...
self.cli = function() {
  program
    ...
    .option('--remove-git', 'Remove .git directories during copy');
...
self.installModule = function(cwd, workspaceDescriptor, packageDescriptor, installed) {
  ...
  console.log("[npm-workspace] Installing direct dependencies " + JSON.stringify(_.keys(allDeps)) + " for " 
  ...
  .then(function() {
      ...
      _.each(deps, function(version, link) {
        promise = promise.then(function() {
          ...
          if(!_.isEmpty(linkPackage.peerDependencies)) {
            ...
            console.log("[npm-workspace] Installing peer dependencies " +
    ...
    console.log("[npm-workspace] npm install "
      + packageDescriptor.name + "@" + packageDescriptor.version + " for " + cwd);
    return self.npm(['install'], cwd);
  });
};
...
self.npm = function(args, cwd) {
  ...
  if(program.verbose) {
    ...
    outStream = new streamBuffers.WritableStreamBuffer();
  }

  npm.stdout.pipe(outStream);
  npm.stderr.pipe(outStream);

  npm.on('close', function (code) {
    if(code !== 0) {
      if(!program.verbose) {
        //output the buffered stream in stderr
        console.error(outStream.getContentsAsString("utf8"));
...
self.installWorkspaceDependencies = function(cwd, dependencies, workspaceDescriptor, linkOnly, installed) {
  ...
  _.each(dependencies, function(version, name) {
    promise = promise.then(function() {
      console.log("[npm-workspace] Processing module " + name + "@" + version + " for module " + cwd);
      var mapping = workspaceDescriptor.links[name];
      var dest = path.resolve(nodeModulesDir, name);
      if(mapping) {
        console.log("[npm-workspace] Found mapping for module " + name + "@" + version);
        var promise = when.resolve();
          ...
          if(!fs.existsSync(dest)) {
            ...
            promise = deferred.promise.then(function() {
              ...
              if(program['removeGit']) {
                console.log("[npm-workspace] Cleaning .git directory " + path.join(dest, '.git'));
        ...
        if(!fs.existsSync(dest)) {
          console.log("[npm-workspace] Installing single module "+ name+"@"+version);
