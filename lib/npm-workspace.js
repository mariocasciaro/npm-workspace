 ...
 */
function isScoped(name){return (name.indexOf('@') >=0 && name.indexOf('/')>=0 );}
function getScope(name){return name.split('/')[0];}
function ensureScopePathExists(modulesDir, scope) {
  var dir = path.resolve(modulesDir, scope);
  if(!fs.existsSync(dir)) {
    fs.mkdirSync(dir);
    self.log.info("Created directory for scope "+ dir );
  }
}
...
self.installWorkspaceDependencies = function(cwd, dependencies, workspaceDescriptor, installed) {
  ...
  dependencies.forEach(function(spec){
    ...
    if (links[spec.name]) {
      linkDependencies.push({name:spec.name, version:spec.version, mapping:links[spec.name], modulesDir:nodeModulesDir, dest:dest});
    } else if (repos[spec.name]) {
      specialRepoDeps.push({name:spec.name, version:spec.version, altRepository:repos[spec.name], modulesDir:nodeModulesDir, dest:dest});
    } else {
      regularRepoDeps.push({name:spec.name, version:spec.version, modulesDir:nodeModulesDir, dest:dest});
    }
  });
  ...
  linkDependencies.forEach(function(item) {
    promise = promise.then(function() {
      ...
      if(program.copy) {
        ...
        if(isScoped(item.name)) {
          ensureScopePathExists(item.modulesDir,getScope(item.name));
        }
