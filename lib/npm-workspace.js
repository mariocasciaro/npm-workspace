...
self.install = function(cwd, installed) {
  ...
  if(wsDesc) {
    ret = self.installWorkspace(cwd, wsDesc, installed);
  }
...
self.installWorkspace = function(cwd, workspaceDescriptor, installed) {
  console.log("[npm-workspace] Installing workspace" + cwd);
  installed = installed || [];
  if(!workspaceDescriptor) {
    workspaceDescriptor = self.getWorkspaceDescriptor(cwd, true);
  }
...
self.installModule = function(cwd, workspaceDescriptor, packageDescriptor, installed) {
  ...
  console.log("[npm-workspace] Installing module " + packageDescriptor.name + "@" + packageDescriptor.version);
  ...
  .then(function() {
    return self.npm(['install'], cwd).then(function() {
      ...
      var links = self.getLinks(nodeModulesDir);
      ...
      function processLinked(deps) {
        ...
        return promise.then(function() {
          var diff = _.difference(newDeps, links);
          //update the global list
          Array.prototype.push.apply(links, diff);
          //process only new links
          return processLinked(diff);
        });
      }

      return processLinked(links);
    });
  });
};
    ...
    promise = promise.then(function() {
      ...
      if(mapping) {
        ...
        if(!fs.existsSync(linkDest)) {
          ...
          return self.install(linkDest, installed).then(function() {
